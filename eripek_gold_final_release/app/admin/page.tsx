...
const RPC_BASE = 'https://txknydpygsvwdhxoumcm.supabase.co/rest/v1/rpc/'
...
type ServiceItem = { ticket_no:string; status:string; issue_type:string; description?:string; appointment_at?:string|null; admin_note?:string|null; created_at:string; customer_name:string; phone:string; block:string; floor:string; unit_no:string; attachments?:ServiceAttachment[] }
type ProjectItem = { request_no:string; status:string; request_type?:string; room?:string; design_name?:string; material_name?:string; notes?:string; appointment_at?:string|null; admin_note?:string|null; quote_amount?:number|null; quote_currency?:string|null; quote_note?:string|null; quote_valid_until?:string|null; quoted_at?:string|null; created_at:string; customer_name:string; phone:string; block:string; floor:string; unit_no:string }
...
async function sha256Hex(value:string){const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(value));return Array.from(new Uint8Array(digest)).map(v=>v.toString(16).padStart(2,'0')).join('')}
async function adminRpc(name:string, body:Record<string,unknown>){const res=await fetch(`${RPC_BASE}${name}`,{method:'POST',headers:{'Content-Type':'application/json',apikey:PUBLIC_KEY,Authorization:`Bearer ${PUBLIC_KEY}`},body:JSON.stringify(body),cache:'no-store'});const data=await res.json().catch(()=>null);if(!res.ok||data!==true)throw new Error('rpc_failed');return true}
...
function localInputValue(v?:string|null){if(!v)return '';const d=new Date(v);const pad=(n:number)=>String(n).padStart(2,'0');return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`}
...
function AdminProjects({items,session,onUpdated,setMsg}:{items:ProjectItem[];session:string;onUpdated:()=>Promise<void>;setMsg:(v:string)=>void}){
  ...
  return <div className="adminList">{items.map(x=><div className="adminTicket" key={x.request_no}><div className="adminTicketTop"><div><div className="ticketNo">{x.request_no}</div><strong>{x.customer_name}</strong><div className="small muted">{x.block} Blok • {x.floor}. Kat • Daire {x.unit_no} • {x.phone}</div></div><span className={`statusBadge st-${x.status}`}>{PROJECT_STATUS[x.status]||x.status}</span></div><div className="adminDetail"><b>{x.request_type||'Proje talebi'}</b><p>{[x.room,x.design_name,x.material_name].filter(Boolean).join(' • ')}</p>{x.notes&&<p className="muted">“{x.notes}”</p>}{x.quote_amount ? <div className="adminQuoteSummary"><span>Müşteriye görünen teklif</span><strong>{new Intl.NumberFormat('tr-TR',{style:'currency',currency:'TRY'}).format(Number(x.quote_amount))}</strong>{x.quote_note&&<small>{x.quote_note}</small>}{x.quote_valid_until&&<small>Geçerlilik: {new Intl.DateTimeFormat('tr-TR').format(new Date(`${x.quote_valid_until}T12:00:00`))}</small>}</div>:null}<small>{dateTR(x.created_at)}</small></div><RequestPlanningEditor kind="project" itemNo={x.request_no} appointmentAt={x.appointment_at} adminNote={x.admin_note} session={session} onUpdated={onUpdated} setMsg={setMsg}/><QuoteEditor item={x} session={session} onUpdated={onUpdated} setMsg={setMsg}/><StatusSelect value={x.status} labels={PROJECT_STATUS} onChange={async status=>{await gateway({action:'admin_project_update',admin_session:session,request_no:x.request_no,status});setMsg(`${x.request_no} güncellendi.`);await onUpdated()}}/></div>)}</div>
}

function RequestPlanningEditor({kind,itemNo,appointmentAt,adminNote,session,onUpdated,setMsg}:{kind:'service'|'project';itemNo:string;appointmentAt?:string|null;adminNote?:string|null;session:string;onUpdated:()=>Promise<void>;setMsg:(v:string)=>void}){
  const [appointment,setAppointment]=useState(localInputValue(appointmentAt));const [note,setNote]=useState(adminNote||'');const [busy,setBusy]=useState(false)
  async function save(){setBusy(true);try{await adminRpc(kind==='service'?'edge_admin_set_service_details':'edge_admin_set_project_details',{p_session_hash:await sha256Hex(session),[kind==='service'?'p_ticket_no':'p_request_no']:itemNo,p_appointment_at:appointment?new Date(appointment).toISOString():null,p_admin_note:note});setMsg(`${itemNo} için planlama bilgisi müşteriye kaydedildi.`);await onUpdated()}catch{setMsg('Planlama bilgisi kaydedilemedi.')}finally{setBusy(false)}}
  return <div className="adminPlanningEditor"><label className="label">Randevu / müşteri bilgilendirmesi</label><input className="input" type="datetime-local" value={appointment} onChange={e=>setAppointment(e.target.value)}/><textarea className="input textarea" rows={2} value={note} onChange={e=>setNote(e.target.value)} placeholder="Müşterinin Taleplerim ekranında göreceği not (isteğe bağlı)"/><button className="adminPlanningBtn" type="button" disabled={busy} onClick={save}>{busy?'Kaydediliyor…':'Planlamayı Kaydet'}</button></div>
}
