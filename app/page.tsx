...
const REFERENCE_ATLAS_COLS = 5
const REFERENCE_ATLAS_ROWS = 8
const REFERENCE_ATLAS_MAP: Record<string, {x:number;y:number}> = {"IMG_5318-thumb.webp":{"x":0,"y":0},"IMG_5319-thumb.webp":{"x":1,"y":0},"IMG_5320-thumb.webp":{"x":2,"y":0},"IMG_5321-thumb.webp":{"x":3,"y":0},"IMG_5324-thumb.webp":{"x":4,"y":0},"IMG_5326-thumb.webp":{"x":0,"y":1},"IMG_5328-thumb.webp":{"x":1,"y":1},"IMG_5330-thumb.webp":{"x":2,"y":1},"IMG_5331-thumb.webp":{"x":3,"y":1},"IMG_5332-thumb.webp":{"x":4,"y":1},"IMG_5334-thumb.webp":{"x":0,"y":2},"IMG_5335-thumb.webp":{"x":1,"y":2},"IMG_5338-thumb.webp":{"x":2,"y":2},"IMG_5339-thumb.webp":{"x":3,"y":2},"IMG_5341-thumb.webp":{"x":4,"y":2},"IMG_5343-thumb.webp":{"x":0,"y":3},"IMG_5344-thumb.webp":{"x":1,"y":3},"IMG_5347-thumb.webp":{"x":2,"y":3},"IMG_5349-thumb.webp":{"x":3,"y":3},"IMG_5350-thumb.webp":{"x":4,"y":3},"IMG_5351-thumb.webp":{"x":0,"y":4},"IMG_5352-thumb.webp":{"x":1,"y":4},"IMG_5355-thumb.webp":{"x":2,"y":4},"IMG_5356-thumb.webp":{"x":3,"y":4},"IMG_5357-thumb.webp":{"x":4,"y":4},"IMG_5359-thumb.webp":{"x":0,"y":5},"IMG_5360-thumb.webp":{"x":1,"y":5},"IMG_5361-thumb.webp":{"x":2,"y":5},"IMG_5367-thumb.webp":{"x":3,"y":5},"IMG_5377-thumb.webp":{"x":4,"y":5},"IMG_5379-thumb.webp":{"x":0,"y":6},"IMG_5380-thumb.webp":{"x":1,"y":6},"IMG_5382-thumb.webp":{"x":2,"y":6},"IMG_5383-thumb.webp":{"x":3,"y":6},"IMG_5385-thumb.webp":{"x":4,"y":6},"IMG_5386-thumb.webp":{"x":0,"y":7},"IMG_5388-thumb.webp":{"x":1,"y":7},"IMG_5390(1)-thumb.webp":{"x":2,"y":7},"IMG_5391-thumb.webp":{"x":3,"y":7}}

function ReferenceAsset({src,alt,className=''}:{src:string;alt:string;className?:string}) {
  const file = src.split('/').pop() || ''
  const pos = REFERENCE_ATLAS_MAP[file]
  if (!pos) return <span className={`referenceAtlasMissing ${className}`} role="img" aria-label={alt} />
  const x = REFERENCE_ATLAS_COLS === 1 ? 0 : (pos.x / (REFERENCE_ATLAS_COLS - 1)) * 100
  const y = REFERENCE_ATLAS_ROWS === 1 ? 0 : (pos.y / (REFERENCE_ATLAS_ROWS - 1)) * 100
  return <span
    className={`referenceAtlasAsset ${className}`}
    role="img"
    aria-label={alt}
    style={{
      backgroundImage: "url('/references-atlas.jpg')",
      backgroundSize: `${REFERENCE_ATLAS_COLS * 100}% ${REFERENCE_ATLAS_ROWS * 100}%`,
...
function DiscoverTab({ residence, sessionToken, favorites, cart, studioVariants, selection, onSelectionChange, onRefresh }: { residence: Residence; sessionToken: string; favorites: FavoriteItem[]; cart: CartItem[]; studioVariants: StudioVariant[]; selection: DiscoverSelection; onSelectionChange: (next: DiscoverSelection, mode?: HistoryMode) => void; onRefresh: () => Promise<void> }) {
  ...
  return <>
    <style jsx global>{`
      ...
      .referenceAtlasAsset{display:block;width:100%;background-repeat:no-repeat;background-color:#e8e0d5}.referenceCard .referenceAtlasAsset{aspect-ratio:1.18/1}.referenceHeroImage{display:block;width:100%;aspect-ratio:4/3;max-height:54dvh;background-repeat:no-repeat;background-color:#161412}.referenceThumb .referenceAtlasAsset{aspect-ratio:1/1}.referenceAtlasMissing{display:block;width:100%;aspect-ratio:4/3;background:#e7ded3}
    ...
    {viewer === 'reference' && selectedReference && <div className="referenceDetailOverlay" role="dialog" aria-modal="true" aria-label={`${selectedReference.title} proje detayı`} onClick={e => { if (e.target === e.currentTarget) { setViewer(null); setSelectedReference(null); setReferenceIndex(0) } }}><div className="referenceDetailSheet">
      ...
      <div className="referenceHeroWrap">{selectedReferenceImage && <ReferenceAsset className="referenceHeroImage" src={referenceThumbPath(selectedReferenceImage)} alt={`${selectedReference.title} uygulama görseli ${referenceIndex + 1}`}/>}<span className="referenceCounter">{referenceIndex + 1}/{selectedReference.gallery.length}</span>{selectedReference.gallery.length > 1 && <div className="referenceNav"><button type="button" aria-label="Önceki görsel" onClick={() => setReferenceIndex(i => (i - 1 + selectedReference.gallery.length) % selectedReference.gallery.length)}>‹</button><button type="button" aria-label="Sonraki görsel" onClick={() => setReferenceIndex(i => (i + 1) % selectedReference.gallery.length)}>›</button></div>}</div>
      {selectedReference.gallery.length > 1 && <div className="referenceThumbs">{selectedReference.gallery.map((img, i) => <button type="button" key={`${img}-${i}`} className={i === referenceIndex ? 'referenceThumb active' : 'referenceThumb'} onClick={() => setReferenceIndex(i)} aria-label={`${i + 1}. görsel`}><ReferenceAsset src={referenceThumbPath(img)} alt={`${selectedReference.title} küçük önizleme ${i + 1}`}/></button>)}</div>}
